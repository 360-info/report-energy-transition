...
country_points <-
  ...
  mutate(iso = recode(country, "Kosovo" = "XKX", .default = iso))
```
...
irena_joined %>%
  ...
  select(iso, country, year, starts_with("all"), starts_with("prop")) ->
proportions
```
...
proportions %>%
  write_csv(here("data", "irena-processed.csv")) %>%
  pivot_wider(
    names_from = year,
