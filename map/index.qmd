...
filteredData = allData.filter(t => t.year == Number(selectedYear));
...
energyBubbles.setProps({
  data: filteredData,
  // because the scales need to be told to point to a different data column
  // depending on the selected controls, we also update them here
  getFillColor:
    d => divergingSolorScale(d["renewprop_" + selectedMeasure]),
  getRadius: d => Math.sqrt(d["total_" + selectedMeasure])
});
